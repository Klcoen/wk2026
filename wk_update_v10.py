...
def wedstrijden_sectie(alle_wedstrijden):
    ...
    return (
        ...
        '<p class="toelichting">De laatste %d gespeelde en de %d eerstvolgende '
        'wedstrijden; lopende duels staan ertussen. Tijden in NL-tijd.</p>'
        '%s'
        '</div>' % (live_badge, VENSTER_AANTAL, VENSTER_AANTAL, inhoud)
    )
...
def bouw_html(groepen, derde_plaatsen, groep_wedstrijden, knockout, tijd,
    ...
    if foutmelding:
        ...
        body = (
            ...
            '</nav>'
            '<section id="tab-wedstrijden" class="tab-panel active">%s</section>'
            '<section id="tab-groepen" class="tab-panel">%s</section>'
            '<section id="tab-knockout" class="tab-panel">%s</section>'
            % (wedstrijden_html, groepen_html, knockout_html)
        )
...
<title>WK 2026 &mdash; De tussenstand</title>
...
.vd-context {{ color: var(--bp-muted); font-size: .72rem; text-align: right; line-height: 1.25; }}
@media (max-width: 560px) {{
  .vd-match {{ grid-template-columns: 64px 1fr auto 1fr; }}
...
<body>
<header>
  <h1>WK 2026 &mdash; De tussenstand</h1>
