...
def oranje_routes(groepen, doelland="Nederland"):
    """Geeft de set wedstrijdnummers (Laatste 32 t/m finale) waarin `doelland`
    terecht zou kunnen komen: alle R32-instappunten (als nr. 1, nr. 2 of beste
    nummer 3 van zijn groep) plus alles wat daar stroomafwaarts uit volgt.
    Leeg als het land niet in de groepen voorkomt."""
    groep = None
    for letter, rijen in (groepen or {}).items():
        if any(r.get("naam") == doelland for r in rijen):
            groep = letter
            break
    if not groep:
        return set()
    ...
    for s in R32_SCHEMA:
        for slot in (s["thuis"], s["uit"]):
            typ = slot[0]
            if typ in ("winner", "runner") and slot[1] == groep:
                nrs.add(s["nr"])
            elif typ == "third" and groep in slot[1]:
                nrs.add(s["nr"])
    # Stroomafwaarts: een bracket-duel is bereikbaar zodra een van zijn feeders
    # uit een al-bereikbaar duel komt (winnaar óf verliezer -> ook de troostfinale).
    veranderd = True
    while veranderd:
        veranderd = False
        for b in KO_BRACKET:
...
def bouw_html(groepen, derde_plaatsen, groep_wedstrijden, knockout, tijd,
    ...
    if foutmelding:
        ...
        knockout_html = knockout_sectie(knockout, last32_proj, ambigu,
                                        oranje_nrs=oranje_routes(groepen))
