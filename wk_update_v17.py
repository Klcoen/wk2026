...
# -*- coding: utf-8 -*-
"""
WK Speelschema 2026  (v17)
==========================
v17: oranje routes bewegen nu mee met de gespeelde wedstrijden. Zolang de groep
nog loopt: alle mogelijke instappunten. Groep beslist: alleen Nederlands echte
Laatste-32-duel + downstream. Uitgeschakeld in de groep: geen oranje meer.
v16: alle knockout-wedstrijden waar Nederland in terecht zou kunnen komen worden
oranje gemarkeerd (oranje_routes): de R32-instappunten als nr. 1 / nr. 2 / beste
nummer 3 van zijn groep, plus alles wat daar stroomafwaarts uit volgt.
v15: paginarefresh van 15 min naar 90 sec (REFRESH_SECONDS=90), zodat de pagina
bij de bezoeker bijna live meeloopt. Past samen met een snellere deploy-cadans
(cron-job.org elke ~2 min). Refresh-tekst toont nu seconden of minuten.
...
REFRESH_SECONDS = 90        # browserpagina herlaadt zichzelf (90 sec ~ live)
...
def proj_match_block(m, oranje=False):
...
def bracket_match_block(m, oranje=False):
    ...
    return (
        ...
        '</div>' % (' oranje' if oranje else '',
                    m["nr"], html.escape(m["tijd"]), badge, thuis, mid, uit)
    )


def _forward_close(entry):
    """Breidt een set R32-wedstrijdnummers uit met alle stroomafwaartse duels:
    een bracket-duel hoort erbij zodra een van zijn feeders uit een al-opgenomen
    duel komt (winnaar óf verliezer -> ook de troostfinale)."""
    nrs = set(entry)
    veranderd = True
    while veranderd:
        veranderd = False
        for b in KO_BRACKET:
            if b["nr"] in nrs:
                continue
            if b["thuis"][1] in nrs or b["uit"][1] in nrs:
                nrs.add(b["nr"])
                veranderd = True
    return nrs


def oranje_routes(groepen, last32_proj=None, doelland="Nederland"):
    """Geeft de set wedstrijdnummers (Laatste 32 t/m finale) waarin `doelland`
    ...
    """
    groep_letter, nl_rij = None, None
    for letter, rijen in (groepen or {}).items():
        for r in rijen:
            if r.get("naam") == doelland:
                groep_letter, nl_rij = letter, r
                break
        if groep_letter:
            break
    if not groep_letter:
    ...
    rijen = groepen[groep_letter]
    groep_beslist = all(r.get("gespeeld", 0) >= 3 for r in rijen)

    # Uitgeschakeld in de groep (4e, of 3e buiten de beste 8): geen routes meer.
    if groep_beslist and nl_rij.get("status") == "uit":
        return set()

    if groep_beslist:
        # Definitief lot bekend: pak het echte Laatste-32-duel van het land.
        entry = {d["nr"] for d in (last32_proj or [])
                 if doelland in (d["thuis_team"].get("name", ""),
                                 d["uit_team"].get("name", ""))}
        if not entry:
            return set()
    else:
...
def knockout_sectie(knockout, last32_proj, ambigu, oranje_nrs=None):
    ...
    for stage, titel in KO_RONDES:
        if stage == "LAST_32" and last32_proj:
            blokken = "".join(proj_match_block(m, m["nr"] in oranje_nrs)
                              for m in last32_proj)
        elif stage in later:
            blokken = "".join(bracket_match_block(m, m["nr"] in oranje_nrs)
                              for m in later[stage])
        else:
    ...
    return (
        ...
        'uitslagen bekend zijn. Alle tijden in NL-tijd.%s%s</p>'
        '<div class="ko-bracket">%s</div>'
        '</div>' % (extra, oranje_tekst, "".join(kolommen))
    )
...
def bouw_html(groepen, derde_plaatsen, groep_wedstrijden, knockout, tijd,
    ...
    if foutmelding:
        ...
        knockout_html = knockout_sectie(knockout, last32_proj, ambigu,
                                        oranje_nrs=oranje_routes(groepen, last32_proj))
...
/* Oranje: een wedstrijd waar Nederland in terecht zou kunnen komen */
.ko-match.oranje {{
  background: #FFF1E2; border-color: var(--oranje);
  box-shadow: inset 4px 0 0 var(--oranje);
}}
.oranje-mark {{
  color: #fff; background: var(--oranje); border-radius: 4px;
...
<header>
  ...
  <p class="intro">Een live overzicht van het WK 2026: welke landen er bij deze stand doorgaan naar de laatste 32, de volledige groepsstanden en het knock-outschema t/m de finale. Kies hieronder een tabblad. De pagina ververst zichzelf elke {refresh_tekst}; alle tijden in NL-tijd.</p>
</header>
{body}
...
document.querySelectorAll('.tab-btn').forEach(function (knop) {{
  knop.addEventListener('click', function () {{
    ...
    if (doel) {{ doel.classList.add('active'); }}
        ...
        refresh_tekst=("%d seconden" % REFRESH_SECONDS if REFRESH_SECONDS < 120
                       else "%d minuten" % (REFRESH_SECONDS // 60)),
